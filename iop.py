...
from config import LOGS_PATH, JSON_PATH, FOLDER_ID, TELEGRAM_TOKEN, IAM_TOKEN_PATH, VJOSN_PATH
...
class IOP:
    ...
    def tts(self, message: telebot.types.Message) -> bool | tuple[bool, str]:
        ...
        if len(text) > 0 and len(text) < 251 and len(text) < int(self.db[str(id)]["limit"]):
    ...
    def get_inline_keyboard(self, values: tuple[tuple[str, str]]) -> telebot.types.InlineKeyboardMarkup:
        markup = telebot.types.InlineKeyboardMarkup()
        for value in values:
            markup.add(telebot.types.InlineKeyboardButton(text=value[0], callback_data=value[1]))
        return markup
    
    def get_reply_markup(self, values: tuple[str]) -> telebot.types.ReplyKeyboardMarkup | None:
        if values:
...
class SpeechKit:
    ...
    def text_to_speech(text: str, id: str):
        ...
        if response.status_code == 200:
            ...
            return False, f"При запросе в SpeechKit возникла ошибка c кодом: {response.status_code}"
